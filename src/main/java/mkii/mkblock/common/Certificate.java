...
import static mkii.mkblock.common.Util.isAllZeroes;
...
public class Certificate {
    ...
    public String getMinCertificateScoreWithNonce(){
        OUTPRT("Checking nonce's upto " + maxNonce + "...");
        ...
        try{
            ...
            for (int i = 0; i < maxNonce; i++) {
                ...
                if( i%10000 == 0) OUTPRT("---> [ " + maxNonce + " / " + i + " ]");
        ...
        OUTPRT("Best nonce:" + bestNonce + " with score: " + score);
    ...
    public long getScoreAtNonce(int nonce){
        try{
            ...
            OUTPRT("Unable to get score at nonce " + nonce + " on a certificate!");
            OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to get score at nonce " + nonce + " on a certificate!" + ANSI_RESET);
        }
