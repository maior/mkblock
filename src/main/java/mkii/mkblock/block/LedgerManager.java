...
public class LedgerManager {
    ...
    public LedgerManager(String addrDb) {
        ...
        if (this.addrDb.exists()) {
            try {
                ...
                while (sc.hasNextLine()) {
                    ...
                    if (in.contains(":")) {
                        ...
                        OUTPRT("addr : " + parts[0]);
                        OUTPRT("balance : " + parts[1]);
                        OUTPRT("currentSigCount : " + parts[2]);
                        String addr = parts[0];
                ...
                OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to read address database file!" + ANSI_RESET);
                OUTPRT("Unable to read address database file!");
                e.printStackTrace();
                System.exit(-1);
            }
        } else {
            OUTPRT("not addrDb.exists !!!! : " + this.addrDbName);
            ...
            try {
                ...
                OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to write to the ledger record file!");
                e.printStackTrace();
                OUTPRT("Unable to write to the ledger record file!");
                System.exit(-1);
            }
            OUTPRT("Creating \"" + addrDbName + "\" database file...");
        }
    }
    ...
    public String getLedgerHash() {
        ...
        try {
            ...
            OUTPRT(ANSI_RED + "Unable to generate ledger hash. Exiting...");
            e.printStackTrace();
            System.exit(-1);
        }
    ...
    public boolean executeTransaction(String tx) {
        try {
            ...
            if (destAmounts.size() != destAmounts.size()) {
                OUTPRT("YOU SHOULD NOT SEE THIS. IF YOU DO< THIS IS VERY BAD");
                return false;
            }
        ...
        try {
            ...
            for (int i = 0; i < destinationAmounts.size(); i++) {
                if (getAddressBalance(destinationAddresses.get(i)) < destinationAmounts.get(i)) {
                    OUTPRT("[CRITICAL ERROR] ADDRESS " + destinationAddresses.get(i) + " needs to return " + destinationAmounts.get(i) + " but only has " + getAddressBalance(destinationAddresses.get(i))); //BIG PROBLEM THIS SHOULD NEVER HAPPEN
    ...
    public boolean writeToFile() {
        try {
            ...
            OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to write to db file!" + ANSI_RESET);
            e.printStackTrace();
            OUTPRT("Unable to write to db file!");
            return false;
        }
        return true;
    }
