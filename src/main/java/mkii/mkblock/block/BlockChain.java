...
public class BlockChain {
    ...
    public boolean addBlock(Block block, boolean fromChainFile) {
        OUTPRT("Attempting to add block " + block.blockIndex + " with hash " + block.blockHash);
        try {
            boolean isPoS = false;
            ...
            if (block.difficulty != 150000 && !isPoS) {
                OUTPRT("Block detected with wrong difficulty");
                return false;
            }
            ...
            if (!block.validateBlock(this)) {
                OUTPRT(" [DAEMON] - " + ANSI_RED + "Failed to validate block!" + ANSI_RESET);
                return false; // If it's not a valid block, don't add it of course
            }

            if (block.blockIndex > largestChainLen) {
                queue.add(block);
                OUTPRT("Block " + block.blockIndex + " with starting hash " + block.blockHash.substring(0, 8) + " added to queue.");
                OUTPRT("Largest chain length: " + largestChainLen);
                OUTPRT("Block index: " + block.blockIndex);
                ...
                if (ledgerManager.lastBlockIndex < 0) {
                    ...
                    while (txsToApply.size() > txsApplied && !txsToApply.get(0).equals("")) {
                        ...
                        if (loopCount > 10000) {
                            ...
                            OUTPRT(exitMsg);
                            OUTPRT(" [DAEMON] - " + ANSI_RED + exitMsg + ANSI_RESET);
                            OUTPRT(""+txsToApply.size());
                            System.exit(-1);
                        }
                    }
                }
            ...
            for (int i = 0; i < chains.size(); i++) {
                if (chains.get(i).get(chains.get(i).size() - 1).blockHash.equals(block.blockHash)) {
                    OUTPRT("Duplicate block received from peer, ignoring it.");
                    return false;
                }
            }

            for (int i = 0; i < chains.size(); i++) {
                OUTPRT("Previous block hash according to chain: " + chains.get(0).get(chains.get(i).size() - 1).blockHash);
                OUTPRT("Previous block hash according to added block: " + block.prevBlockHash);
                OUTPRT("Selected chain size: " + chains.get(i).size());
                OUTPRT("Should be equal to block index: " + block.blockIndex);
                ...
                if (chains.get(i).get(chains.get(i).size() - 1).blockHash.equals(block.prevBlockHash) && chains.get(i).size() == block.blockIndex) {
                        ...
                        if (!chains.get(i).get(chains.get(i).size() - 2).blockHash.equals(largestChainLastBlockHash)) {
                            ...
                            for (int j = 0; j < chains.get(i).size(); j++) {
                                ...
                                while (txsToApply.size() > 0) {
                                    ...
                                    for (int k = 0; k < txsToApply.size(); k++) {
                                        OUTPRT("Attempting to execute transaction: " + txsToApply.get(k).substring(0, 45) + "..." + txsToApply.get(k).substring(txsToApply.get(k).length() - 20));
                                        if (ledgerManager.executeTransaction(txsToApply.get(k))) {
                                            OUTPRT("Successfully executed transaction!");
                                            txsToApply.remove(k);
                                            k--; //Compensate for changed ArrayList size
                                        } else {
                                            OUTPRT("Didn't execute transaction...");
                                        }
                                    }
                                    if (loopCount > 10000) {
                                        OUTPRT("Infinite block detected! Hash: " + chains.get(i).get(j).blockHash + " and height: " + chains.get(i).get(j).blockIndex);
                                        System.exit(-1);
                                    }
                                }
                            ...
                            if (ledgerManager.lastBlockIndex < block.blockIndex) {
                                ...
                                while (txsToApply.size() > completedTxs) {
                                    ...
                                    if (loopCount > 10000) {
                                        OUTPRT("Infinite block detected with hash: " + block.blockHash + " at index " + block.blockIndex);
                                        System.exit(-1);
                                    }
                                }
                    ...
                    OUTPRT("Something went wrong with block stacking.");
                    OUTPRT(chains.get(i).get(chains.get(i).size() - 1).blockHash);
                    OUTPRT(block.prevBlockHash);
                }
            }
            ...
            if (!foundPlace) {
                ...
                OUTPRT("Block didn't fit anywhere.");
                return false;
            }
    ...
    public boolean writeBlockToFile(Block block) {
        OUTPRT("Writing a block to the chainfile...");
        try (FileWriter writer = new FileWriter(dbFolder + "/blockchain-DONOTTOUCH.dat", true);
            ...
            OUTPRT("Unable to save block to database (chainfile)!");
            OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to write block to database (" + ioe.getMessage() + ")" + ANSI_RESET);
            return false;
        }
        return true;
    }
    ...
    public boolean saveToFile(String dbFolder) {
        try {
            ...
            OUTPRT("Unable to write blockchain file (" + ioe.getMessage() + ")");
            OUTPRT(" [DAEMON] - " + ANSI_RED + "Unable to write blockchain file" + ANSI_RESET);
            ioe.printStackTrace();
            return false;
        }
        return true;
