...
import mkii.mkblock.common.Util;
...
public class MerkleAddressUtility {
    ...
    public MerkleAddressUtility(){
        try{
            ...
            OUTPRT(Constants.ANSI_RED + "No SHA1PRNG support! Exiting." + Constants.ANSI_RESET);
            System.exit(-1);
        }
        try{
            ...
            OUTPRT(Constants.ANSI_RED + "No SHA-256/512 support. Exiting." + Constants.ANSI_RESET);
            e.printStackTrace();
            System.exit(-1);
        }
        try{
            ...
            OUTPRT(Constants.ANSI_RED + "Unable to create folder for address storage. Exiting." + Constants.ANSI_RESET);
            e.printStackTrace();
            System.exit(-1);
        }
    }
    ...
    public boolean verifyMerkleSignature(String message, String signature, String address, long index){
        try
        {
            ...
            for (int i = 1; i < merkleAuthPathComponents.length - 1; i++) //Go to merkleAuthPathComponents.length - 1 because the final hash is returned in base32 and is truncated
            {
                ...
                if (verbose) {
                    OUTPRT("rollingHash: " + rollingHash + " and auth component: " + merkleAuthPathComponents[i]);
                }
                position /= 2;
            }
    ...
    public String getMerkleSignature(String message, String privateKey, long index, String address){
        File infoFile = new File("addresses/" + address + "/" + INFO_FILENAME);
        if (!infoFile.exists()) {
            return null;
        }
        ...
        //Add a public and private key part to signature for each digit of signable binary
        for (int i = 0; i < binaryToSign.length(); i++) {
            //A zero means we reveal the first private key
            if (binaryToSign.charAt(i) == '0') {
                if (i == binaryToSign.length() - 1) {
                    //If it is part of the last pair, we want to use SHA512 (Full Length)
            ...
            } else if (binaryToSign.charAt(i) == '1') {
                //A one means we reveal the second private key
                if (i == binaryToSign.length() - 1) {
                    //If it is part of the last pair, we want to use SHA512 (Full Length)
                    lamportSignature += SHA512(lamportPrivateKeyParts[i*2]) + ":" + lamportPrivateKeyParts[i*2+1];
                } else {
                    //If it is any one of the other previous pairs, use SHA256Short
                    lamportSignature += SHA256Short(lamportPrivateKeyParts[i*2]) + ":" + lamportPrivateKeyParts[i*2+1];
                }
            } else {
                //Something has gone terribly wrong, our binary string isn't made of binary.
                OUTPRT("CRITICAL ERROR: BINARY STRING IS NOT BINARY");
                System.exit(-4);
            }

            if (i < binaryToSign.length() - 1) {
                //Add a double-colon separator between pairs
                lamportSignature += "::";
            }
        }
        ...
        try {
            ...
            OUTPRT("ERROR: UNABLE TO READ INFORMATION FILE FOR ADDRESS " + address + "!");
            e.printStackTrace();
            return null;
        }
        ...
        OUTPRT("address : " + address + ", authPathIndexes.length : " + authPathIndexes.length);
...
//                for (long j = 0; j < authPathIndexes[i]; j++) {
//                    //Consume authPathIndexes[i] of components to get to the one we want
//                    readLayerFile.nextLine(); //Nom-nom
//                }

                if (verbose) {
                    OUTPRT("Consumed " + authPathIndexes[i] + " from layer " + i + ".");
                }
                String layerData = readLayerFile.nextLine();
                readLayerFile.close();
                if (verbose) {
                    OUTPRT("We think the " + (authPathIndexes[i]) + "th index is " + layerData + ".");
                }

                merklePath += layerData; //readLayerFile.nextLine() will now return the correct hash

                if (i < authPathIndexes.length - 1) {
                    //We want all elements in merklePath to be separated by a colon
                    merklePath += ":";
                }
            } catch (Exception e) {
                OUTPRT("ERROR: UNABLE TO READ ABOUT LAYER " + i + " FROM ADDRESS " + address);
                e.printStackTrace();
                return null;
            }
        }
    ...
    public String[] getLamportPrivateKeyParts(String privateKey, long index){
        try{
            ...
            OUTPRT("Unable to generate lamport key parts. Exiting.");
            e.printStackTrace();
            System.exit(-2);
        }
        return null;
    ...
    public boolean isAddressFormattedCorrectly(String addr){
        try{
            ...
            String prefix = addr.substring(0, 2); // Prefix is at the first 2 chars of the address
